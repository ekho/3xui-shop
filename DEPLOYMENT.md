...
- Команды оператора в топике: `/close` (закрыть тикет), `/ban` / `/unban`, `/info`
  (карточка юзера), `/approve` / `/reject` (решить заявку на регистрацию юзера этого
  топика — работают и без карточки с кнопками). В любом месте группы: `/pending` —
  показать очередь заявок и разложить карточки с кнопками по топикам. Закрытие/переоткрытие
  топика «нативно» в UI Telegram тоже синкает статус тикета. Удалённый вручную топик
  пересоздастся при следующем сообщении юзера.
