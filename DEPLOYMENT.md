...
- Заявки на регистрацию (при `SHOP_APPROVAL_REQUIRED=True`): карточки новых заявок и
  ежечасные напоминания приходят в **General** группы поддержки с кнопками
  Approve/Reject — решить может любой участник группы (кнопки работают и у «анонимных»
  админов). Решение применяет и юзера уведомляет основной бот. Если support-бот
