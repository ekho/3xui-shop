...
**Конвенция:** группа = префикс тега инбаунда в панели до первого дефиса
(`regular-n2-in-8443-tcp` → группа `regular`). Имя группы — строчные латинские
буквы/цифры, **без дефисов**. Ссылки всех инбаундов набора попадают в подписку
автоматически (панель собирает их по `subId` клиента).

**Кто чем владеет:**
- состав группы (какие инбаунды) — теги в панели; удобнее менять из бота:
  Admin Tools → Группы инбаундов → карточка группы → тап по инбаунду (бот сам
  переименует тег). ⚠️ Ретег перезапускает xray; правила xray-роутинга по
  старому `inboundTag` нужно поправить руками;
- какой набор у тарифа — Plans Editor → «Изменить группы» (или `inbound_groups`
  в `plans.json` при первичном переносе);
- какой набор у конкретного юзера — заполняется при покупке из тарифа;
  переопределение: Группы инбаундов → «Группы пользователя» (применяется к
  панели сразу);
- реестр групп — таблица `inbound_groups` в БД бота (создание/переименование/
  удаление — из того же экрана). Бот управляет **только** инбаундами с
  зарегистрированным префиксом: инбаунды с «чужими» тегами (например,
  `n2-in-...`) и ручные прицепки админа к ним не трогаются.

**Reconciler** (крон, раз в час): сводит фактические членства клиентов к
желаемым наборам (attach/detach). Гарантии: никогда не отцепляет клиента до
пустого набора — если группы юзера резолвятся в ноль инбаундов (опечатка в
теге, инбаунд удалён), операция пропускается и разработчику уходит алерт
`#InboundGroupsEmptySet` (дедуп 24 ч).

**Миграция существующей панели на группы:**
1. Обновить бота (миграция БД посеет группу `regular`).
2. В боте: Группы инбаундов → `regular` → включить нужные инбаунды (теги
   станут `regular-<старый тег>`).
3. Существующие юзеры без набора считаются `regular` — первый прогон
   reconciler доцепит их ко всем инбаундам группы. Проверить лог
   `[reconcile]` после первого прогона.
