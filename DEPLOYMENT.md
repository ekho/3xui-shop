...
| `AUDIT_CHANNEL_ID` | нет | — | нет | Приватный канал-зеркало аудит-лога (`-100…`, бот — админ). Пусто → аудит пишется только в БД (`audit_log`). В канал уходят лишь метаданные события + хэштеги (`#uid_…`, `#by_…`, `#act_…`); тела сообщений юзерам живут только в БД. См. §5c. |
| `AUDIT_RETENTION_DAYS` | нет | `365` | нет | Сколько дней хранить события аудит-лога. Суточный джоб (03:30 по `BOT_TIMEZONE`) удаляет старше и логирует это системным событием. |
...
- **История в карточке юзера** (осн. бот, только админ): кнопка «🕓 История действий» в
  ...
  клиенту). Работает всегда, даже без канала. В `/info` support-бота кнопки нет —
  операторы не видят, кто что делал.
- **Telegram-канал-зеркало** (опц., realtime + поиск по хэштегам):
  1. Создать **приватный канал**, добавить основного бота **администратором** (право
     постить сообщения). Оператором аудита он быть не обязан — держите там только админов.
  2. Узнать ID канала (`-100…`, напр. переслав пост боту @getidsbot) → `AUDIT_CHANNEL_ID` в `.env`.
  3. Перезапустить бота.

Поведение и заметки:

- `AUDIT_CHANNEL_ID` пуст → аудит всё равно пишется в БД, просто без зеркала. БД — источник
  истины; ошибка поста в канал никогда не срывает само действие.
- В канал уходят **только метаданные** (актор, действие, target, хэштеги `#uid_…` `#by_…`
  `#act_…`) — поиск по клику на хэштег. **Тела сообщений юзерам** (DM, ответы оператора)
  живут ТОЛЬКО в БД под retention, в канал не попадают.
