...
| BOT_SUPPORT_ID | 🔴 | - | ID пользователя, отвечающего за поддержку (личка-фолбэк, если support-бот не включён) |
| SUPPORT_BOT_TOKEN | ⭕ | - | Токен отдельного support-бота (прокси-поддержка через топики форум-группы); секрет, работает только вместе с SUPPORT_GROUP_ID — см. DEPLOYMENT.md §5b |
| SUPPORT_GROUP_ID | ⭕ | - | ID супергруппы-форума поддержки (-100…, Topics включены, support-бот — админ с can_manage_topics) |
| BOT_DOMAIN | 🔴 | - | Домен вашего бота (например, 3xui-shop.com) |
| BOT_PORT | ⭕ | 8080 | Порт, используемый ботом |
| BOT_TIMEZONE | ⭕ | UTC | Часовой пояс (IANA, напр. Europe/Moscow) для календарных задач; используется месячным сбросом трафика безлимита (1-е число, 00:00) и суточным пруном аудит-лога (03:30) |
| AUDIT_CHANNEL_ID | ⭕ | - | Приватный канал-зеркало аудит-лога (-100…, бот — админ). Пусто → аудит пишется только в БД. В канал уходят лишь метаданные + хэштеги; тела сообщений остаются в БД — см. DEPLOYMENT.md §5c |
| AUDIT_RETENTION_DAYS | ⭕ | 365 | Сколько дней хранить события аудит-лога; суточный джоб чистит старше и пишет об этом системное событие |
| | | |
...
| SHOP_TRIAL_TRAFFIC_GB | ⭕ | 15 | Лимит трафика пробной подписки в ГБ (0 = безлимит) |
...
> Тарифы теперь хранятся в БД и редактируются прямо в боте: **Admin Tools → Plans Editor**
> (создание/удаление тарифа, изменение лимита трафика и цен — без передеплоя). `plans.json` ниже
> используется **только один раз** — для первичного переноса в БД, если она ещё пустая. После
> этого файл больше не читается, дальнейшие правки в нём не действуют.
